  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: IndexedStack(index: _tab, children: [
        ...
        _DoctorArchiveTab(doctorId: widget.doctorId), // ← updated
      ]),
  ...
  Widget build(BuildContext context) {
    return SafeArea(child: CustomScrollView(slivers: [
                ...
                Row(children: [
                  ...
                  GestureDetector(
                      onTap: () => Navigator.push(context,
                          dSlide(PatientListScreen(doctorId: doctorId))),
                      child: Container(
                          ...
                          child: Icon(Icons.search_rounded,
                              size: 18, color: OV.onSurfaceVariant))),
                  const SizedBox(width: 8),
                  // Notifications
                  StreamBuilder<List>(
                      ...
                      builder: (_, snap) {
                        ...
                        return GestureDetector(
                            onTap: () => Navigator.push(context, dSlide(
                                DoctorNotificationsScreen(doctorId: doctorId))),
                  ...
                  GestureDetector(
                      onTap: () => Navigator.push(context,
                          dSlide(DoctorProfileScreen(doctorId: doctorId))),
                      child: Container(
                          width : 36, height: 36,
                          decoration: BoxDecoration(
                              shape: BoxShape.circle,
      ...
      SliverToBoxAdapter(child: StreamBuilder<List<dynamic>>(
          ...
          builder: (_, snap) {
            ...
            final highAlerts = alerts
                .where((a) => a.severity == 'high' && !a.isResolved)
                .toList();
            if (highAlerts.isEmpty) return const SizedBox.shrink();
            final a = highAlerts.first;
            return GestureDetector(
                onTap: () => Navigator.push(context,
                    dSlide(CriticalAlertsScreen(doctorId: doctorId))),
                child: Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        border      : Border.all(
                            color: OV.error.withOpacity(0.3))),
                    child: Row(children: [
                      Container(
                          ...
                          child: Text('Urgent Review',
                              style: GoogleFonts.inter(fontSize: 10,
                                  fontWeight: FontWeight.w700,
                                  color     : Colors.white))),
                      const SizedBox(width: 8),
                      Text(
                          'ID: #${a.patientId.substring(0, 6).toUpperCase()}',
                          style: GoogleFonts.inter(
                              fontSize: 11, color: OV.error)),
                      const Spacer(),
                      Icon(Icons.arrow_forward_rounded,
                          size: 16, color: OV.error),
                    ])));
          })),
          ...
          child: DCard(
              ...
              child: StreamBuilder<List>(
                  ...
                  builder: (_, snap) {
                    ...
                    return Column(
                        children: patients.map((p) => _RecentPatientRow(
                            patient: p,
                            onTap  : () => Navigator.push(context, dSlide(
                                EhrDashboard(        // ← opens EHR directly
                                  patientId: p.uid,
                                  doctorId : doctorId,
                                ))))).toList());
                  })))),
      ...
      SliverToBoxAdapter(child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: Column(
              children: service.getTodaySchedule(doctorId)
      ...
      child: Padding(
          ...
          child: Row(children: [
            ...
            Text(patient.activeDiagnosis ?? '',
                style: GoogleFonts.inter(
                    fontSize: 10, color: OV.onSurfaceVariant),
      ...
      child: IntrinsicHeight(child: Row(children: [
        ...
        Column(children: [
          Container(width: 10, height: 10,
              decoration: BoxDecoration(
                  color: _lineColor, shape: BoxShape.circle)),
          Expanded(child: Container(
              width: 2, color: _lineColor.withOpacity(0.2))),
        ]),
        const SizedBox(width: 12),
        Expanded(child: Container(
            padding: const EdgeInsets.symmetric(
            ...
            child: Column(
              ...
              Text(entry.location, style: GoogleFonts.inter(
                  fontSize: 11, color: OV.onSurfaceVariant)),
            ]))),
      ])));
}
          ...
          child: Row(
              ...
              children: [
                _NavItem(icon : Icons.grid_view_rounded,
                    label: 'Dashboard', sel: current == 0,
                    onTap: () => onTap(0)),
                _NavItem(icon : Icons.people_alt_rounded,
                    label: 'Patients',  sel: current == 1,
                    onTap: () => onTap(1)),
                _NavItem(icon : Icons.calendar_month_rounded,
                    label: 'Schedule',  sel: current == 2,
                    onTap: () => onTap(2)),
                _NavItem(icon : Icons.folder_shared_rounded, // ← updated icon
                    label: 'EHR',       sel: current == 3,
                    onTap: () => onTap(3)),
              ]))));
}
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: Column(
      ...
      Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 0),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('EHR Archive', style: GoogleFonts.manrope(
                fontSize  : 26,
                fontWeight: FontWeight.w700,
                color     : OV.onSurface,
                letterSpacing: -0.3)),
            Text('Tap any patient to open their EHR vault',
                style: GoogleFonts.inter(
          ...
          builder: (_, snap) {
            ...
            if (patients.isEmpty) {
              return Center(child: Column(
                ...
                Text('No patient records yet',
                    style: GoogleFonts.inter(
                        fontSize: 14, color: OV.onSurfaceVariant)),
              ]));
            }
            return ListView.builder(
                ...
                itemBuilder: (_, i) {
                  final p = patients[i];
                  return GestureDetector(
                    // ── Opens EhrDashboard with doctorId ────────
                    onTap: () => Navigator.push(context, dSlide(
                        EhrDashboard(
                          patientId: p.uid,
                          doctorId : doctorId, // ← doctor gets edit access
                        ))),
                    child: Container(
                        ...
                        child: Row(children: [
                          ...
                          Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 4),
                                Text('Open EHR', style: GoogleFonts.inter(
                                    fontSize  : 10,
                                    fontWeight: FontWeight.w600,
                                    color     : OV.primary)),
                              ]),
